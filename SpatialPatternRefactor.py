...
#Save cell coordinate data from Stereo Investigator 
...
##Number of cortical layers, this is used to randomize cell distributions by layer
layer_num = 6
...
def poisson_gen(sp_data, xmin, xmax, ybound_list):
    pois_data = []
    for cell in sp_data:
        yrand = random.uniform(ybound_list[cell[3]-1][0], ybound_list[cell[3]-1][1])
        pois_data.append([cell[0], random.uniform(xmin, xmax), yrand, cell[3]])
    return pois_data
...
def poisson_iterate(prun_num, sp_data_mod, cell1, cell2, xmin, xmax, ymin, ymax, ybound_list):
    poisson_track = []
    for unused in range(0, analysis_dist + 1):
        poisson_track.append(0)
    for runcount in range(0, prun_num):
        print runcount + 1
        poisson_raw = poisson_boundaries(poisson_gen(sp_data_mod, xmin, xmax, ybound_list), xmin, xmax, ymin, ymax)
        if cell1 == cell2:
...
poisson_cluster = poisson_iterate(prun_num, sp_data_mod, cell1, cell2, xmin, xmax, ymin, ymax, ybound_list)
